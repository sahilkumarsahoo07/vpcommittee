          ...
          <div className="flex items-center justify-between gap-3 md:gap-4">
            ...
            <nav className="hidden lg:flex items-center gap-5 xl:gap-8 absolute left-1/2 -translate-x-1/2">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className={`text-sm font-medium text-[#FFF7E8] hover:text-[#F4B942] transition-colors tracking-wide ${fontClass}`}
                >
                  {link.label}
                </a>
              ))}
            ...
            <div className="flex items-center gap-2.5 sm:gap-3 ml-auto">
              ...
              <button
                ...
                className={`hidden md:inline-flex px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider bg-gradient-to-r from-[#F4B942] via-[#D4A72C] to-[#E87516] text-[#32070B] hover:shadow-lg hover:shadow-[#F4B942]/25 hover:scale-105 active:scale-95 transition-all items-center gap-1.5 ${fontClass}`}
              >
              ...
              <button
                ...
                className="lg:hidden w-9 h-9 rounded-lg bg-[#5A0F16]/80 border border-[#D4A72C]/40 text-[#F4B942] flex items-center justify-center"
              >
